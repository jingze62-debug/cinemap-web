/**
 * Import fan-made SIFF schedule Excel into public/data JSON.
 * Usage: node scripts/import-siff-xlsx.mjs [path-to-xlsx]
 */
const fs = require("fs");
...
const xlsxPath =
  process.argv[2] || path.join(ROOT, "tmp_siff_schedule.xlsx");
...
function parseTitle(raw) {
  ...
  while ((m = tagRe.exec(title))) {
    ...
    for (const p of parts) {
      if (/^(4K|Dolby|IMAX|CINITY|LUXE|3D|2D|HDR|Atmos)$/i.test(p) || /修复|纪念/.test(p)) {
        tags.push(p);
      }
    }
  }
  title = title.replace(/\s*\([^)]*\)\s*/g, " ").replace(/\s+/g, " ").trim();
  // also detect trailing tech words without parens rarely
  return { titleZh: title, baseTags: [...new Set(tags)] };
}
...
function parseDate(raw) {
  ...
  // 6/12/26 or 2026-06-12 or 6/12/2026
...
function haversineMin(a, b) {
  ...
  // metro-ish: ~22km/h + 12min transfer buffer
...
function main() {
  ...
  function resolveCinema(rawName) {
    const name = String(rawName).trim();
    if (cinemaNameToId.has(name)) return cinemaNameToId.get(name);
    ...
    if (meta) {
      ...
      console.warn("Unknown cinema mapped:", name, "->", id);
    }
  ...
  for (const r of rows) {
    try {
      ...
      const cinemaId = resolveCinema(r["影院"]);
      ...
      const techTags = [...new Set([...baseTags, ...hallTags])];

      let film = filmsMap.get(titleZh);
      if (!film) {
        const id = `film_${slugify(titleZh) || hashStr(titleZh).toString(36)}`;
        const g = GRADIENTS[hashStr(titleZh) % GRADIENTS.length];
        ...
        film = {
          ...
          year: 2026,
          countries: ["待补充"],
          runtimeMin: minutesBetween(start, end),
          director: "待补充",
          section,
          posterGradient: g,
      ...
      // refine runtime with median-ish first few
      const rt = minutesBetween(start, end);
      if (Math.abs(rt - film.runtimeMin) > 5) {
        film.runtimeMin = Math.round((film.runtimeMin + rt) / 2);
      }
      for (const t of techTags) {
        film._tagVotes[t] = (film._tagVotes[t] || 0) + 1;
      }

      const scrId = `scr_${film.id.replace(/^film_/, "")}_${date.replace(
        /-/g,
        ""
      )}_${start.replace(":", "")}_${cinemaId.replace(/^cinema_/, "")}`;
      // ensure unique
      let uniqueId = scrId;
      let n = 2;
      ...
      film.screenings.push({
        ...
        price,
        techTags,
        scheduledCount: 40 + (hashStr(uniqueId) % 280),
      });

      const c = cinemasById.get(cinemaId);
      c.screeningCount += 1;
    } catch (e) {
      skipped += 1;
      console.warn("skip row", e.message, r);
    }
  }
  ...
  const films = [...filmsMap.values()]
    .map((f) => {
      const { _tagVotes, ...rest } = f;
      if (!rest.poster) delete rest.poster;
      // prefer section with most 4K votes already set
      return rest;
    })
  ...
  // Prefer richer tips from previous cinemas.json when id matches
  ...
  if (fs.existsSync(prevPath)) {
    try {
      ...
      for (const c of cinemasById.values()) {
        ...
        if (p.address && !CINEMA_META[Object.keys(CINEMA_META).find((k) => CINEMA_META[k].id === c.id)]) {
          /* keep new address from meta */
        }
        if (p.address && CINEMA_META) {
          // keep meta address; tips from prev
        }
      }
    } catch {
      /* ignore */
  ...
  const cinemas = [...cinemasById.values()].sort(
  ...
  // preserve a few hand-tuned pairs if both exist
  const tuned = {
    cinema_sfc: { cinema_grand: 32, cinema_tianshan: 22, cinema_majestic: 28, cinema_peace: 30 },
    cinema_grand: { cinema_sfc: 32, cinema_tianshan: 38, cinema_majestic: 18, cinema_peace: 12 },
  };
  ...
  const dataset = {
    ...
    sections: ["全部", "4K修复", "展映精选"],
    films,
    _meta: {
      source: path.basename(xlsxPath),
      importedAt: new Date().toISOString(),
      screeningCount: films.reduce((n, f) => n + f.screenings.length, 0),
  ...
  // strip _meta from public file? Keep for debugging is fine; types don't care.
  // Actually TypeScript FilmsDataset doesn't include _meta - JSON extra field OK at runtime.

  const filmsOut = { ...dataset };
  fs.writeFileSync(
    path.join(ROOT, "public/data/siff_2026_films.json"),
    JSON.stringify(filmsOut, null, 2) + "\n",
    "utf8"
  );
  fs.writeFileSync(
  ...
  console.log(
    JSON.stringify(
      {
        films: films.length,
        screenings: dataset._meta.screeningCount,
        cinemas: cinemas.length,
        skipped,
        topCinemas: cinemas.slice(0, 5).map((c) => `${c.nameZh}:${c.screeningCount}`),
      },
      null,
      2
    )
